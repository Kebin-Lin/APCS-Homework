//Kevin Lin
//APCS2 Pd1
//HW27 -- Make Itr Work   "Make it work," people.
//2018-03-28
...
public class LList<T> implements List<T>
{
  ...
  *****************************************************/
  /* YOUR MODIFIERS HERE */ class MyIterator implements Iterator<T>
  {
    ...
    //               (...so that hasNext() will not crash) <-Is this needed?
    public void remove()
    {
      //Q: What is significance/purpose of these lines?
      //A: Will not try to remove something that isn't there
      if ( ! _okToRemove )
      throw new IllegalStateException("must call next() beforehand");
      _okToRemove = false;

      if ( !hasNext() ) {//Removing head
        removeLast();
      }

      else if ( _dummy.getPrev().getPrev() == null ) {//Removing tail
        removeFirst();
      }
      // Q: How many ELSE-IF's are necessary?
      // Q: What are the cases you must consider?
      // A1: 1
      // A2: Remove head, remove tail, and remove anything other than those
