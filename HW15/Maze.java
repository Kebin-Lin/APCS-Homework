...
1) Check if the current spot is a '$', if so, the maze is solved
2) Move to the next valid spot, set it to a '.'
3) Repeat steps 1-2, if there are no valid spots, return to the previous spot
 ***/
...
class MazeSolver
{
  ...
  public void solve( int x, int y ) {
    ...
    //recursive reduction
      ...
      maze[x][y] = '.';
	    if (x > 0) {solve(x-1,y);}
      if (x < maze.length - 1) {solve(x+1,y);}
      if (y > 0) {solve(x,y-1);}
      if (y < maze[0].length - 1) {solve(x,y+1);}
    }
  }
